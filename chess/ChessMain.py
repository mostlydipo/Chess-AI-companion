...
MAX_FPS = 15 #for later animantions
IMAGES = {}
...
'''
def loadImages():
    pieces = ['wp', 'wR', 'wQ', 'wB', 'wK', 'wN', 'bp', 'bR', 'bQ', 'bB', 'bK', 'bN']
    for piece in pieces:
...
def main():
    ...
    running = True
    sqSelected = () #no square selected, keep track of the user's last click (tuple: (row, col))
    playerClicks = [] #keeps track of player clicks (two tuples: [(6, 4), (4,4])
    gameOver = False
    playerOne = True #if a human is playing white then this would be true, if an AI is playing, then false
    ...
    while running:
        ...
        if not gameOver and not humanTurn and not moveUndone:
            if not AIThinking:
                ...
                moveFinderProcess.start() #call findBestMoves
...
'''
Draw the squares on the board. The tip left square is always light.
'''
...
'''
Highlight the selected square and possible moves for selected piece
'''
...
'''
Draw the pieces on the board using the current Gamestate.bord
'''
...
def drawMoveLog(screen, gs, font):
    ...
    for i in range(0, len(moveLog), 2):
        ...
        if i+1 < len(moveLog):
            moveString += moveLog[i].getChessNotation()
